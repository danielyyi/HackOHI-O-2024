    ...
    while True:
        ...
        # Writing the on screen data on the frame
            if SHOW_ON_SCREEN_DATA:
                ...
                cv.putText(frame, f"Total Sleeping Time: {math.floor((SLEEP_TIME/30)*10)/10}", (30, 140), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                ...
                if ENABLE_HEAD_POSE:
                    ...
                    cv.putText(frame, f"Pitch: {int(pitch)}", (30, 170), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                    cv.putText(frame, f"Yaw: {int(yaw)}", (30, 200), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                    cv.putText(frame, f"Roll: {int(roll)}", (30, 230), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
